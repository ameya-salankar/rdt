...
class Client:
    ...
    def receive(self):
        ...
        # Send SYN packets
        while recvd == False:
            timed_send = threading.Thread(target=self.timer, args=(msg, self.server_address, True), daemon=True)
            timed_send.start()
            while True:
                try:
        ...
        if recvd == True:
            ...
            if header["purpose"] == "ACK":
                ...
                while True:
                    self.terminate = False
                    timed_send = threading.Thread(target=self.timer, args=(msg, self.server_address), daemon=True)
                    timed_send.start()
                    resp = self.receive_socket.recvfrom(self.buffer_size)
                    ...
                    if not corrupted:    
                        ...
                        if header['purpose'] == "ACK" and int(header['seqno']) == self.expected_seq_no:
